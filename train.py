...
def train(epoch):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        loss = F.mse_loss(model(data)[data.target_class == 0], data.y[data.target_class == 0])
        loss.backward()
...
def test(loader):
    ...
    for data in loader:
        data = data.to(device)
        error += (model(data)[data.target_class == 0] * std - data.y[data.target_class == 0] * std).abs().sum().item()  # MAE
