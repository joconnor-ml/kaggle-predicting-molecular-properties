...
def readout_edgewise(X, latent_size):
    # X : [#Batch, #Atom, #Feature] --> Z : [#Batch, #Atom, #Latent] -- reduce_sum --> [#Batch, #Latent]
    hidden_dim = [latent_size, latent_size, latent_size, 1]
    num_atoms = int(X.get_shape()[1])
    ...
    weight = {
        ...
        'mlp_f3': tf.get_variable("fw3", initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[hidden_dim[2], hidden_dim[3]], dtype=tf.float64),
    }
    bias = {
        'mlp_f0': tf.get_variable("fb0", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[0]],
                                  dtype=tf.float64),
        'mlp_f1': tf.get_variable("fb1", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[1]],
                                  dtype=tf.float64),
        'mlp_f2': tf.get_variable("fb2", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[2]],
                                  dtype=tf.float64),
        'mlp_f3': tf.get_variable("fb3", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[3]],
                                  dtype=tf.float64),
    }

    # Graph Embedding in order to satisfy invariance under permutation
    atom_representation = tf.einsum('ijk,kl->ijl', X, weight['mlp_f0']) + tf.reshape(
        tf.tile(bias['mlp_f0'], [num_atoms]), [num_atoms, hidden_dim[0]])

    # atom_representation.shape = [num_atoms, hidden_dim]
    # edge_representation.shape = [num_atoms, num_atoms, hidden_dim]

    # now let's combine into edge representation
    atom1 = tf.tile(atom_representation, num_atoms)
    atom2 = tf.reshape(tf.tile(tf.expand_dims(atom_representation, axis=-1), num_atoms), shape=[-1])
